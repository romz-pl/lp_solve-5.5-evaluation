...
void code_free(CodeNode* node)
{
   int i, j, n = 0, n0, level, level1, levelmax = 0, nmax = 1;
   struct nodes2free {
     int level;
     CodeNode *node;
   } *nodes2free;
#  define step 10
   ...
   n0 = step;
   nodes2free = malloc(n0 * sizeof(*nodes2free));
   nodes2free[n].level = 0;
   nodes2free[n++].node = node;
   level = -1;
   while (n > 0) {
     level1 = level;
     level = nodes2free[n - 1].level;
     node = nodes2free[n - 1].node;
     if(level >= level1) {
       level1 = level;
       for(i = MAX_CHILDS - 1; i >= 0; i--)
         if (node->child[i] != NULL) {
	   level = level1 + 1;
	   if(level > levelmax)
	     levelmax = level;
	   if(n == n0) {
	     n0 += n0 /* step */;
	     nodes2free = realloc(nodes2free, n0 * sizeof(*nodes2free));
	   }
	   nodes2free[n].level = level;
           nodes2free[n++].node = node->child[i];
	   if(n > nmax)
	     nmax = n;
	 }
     }
