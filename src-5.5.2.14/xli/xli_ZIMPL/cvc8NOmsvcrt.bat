...
%c% -DVERSION="""%ZIMPL_MAJORVERSION%.%ZIMPL_MINORVERSION%""" -DZIMPL_MAJORVERSION=%ZIMPL_MAJORVERSION% -DZIMPL_MINORVERSION=%ZIMPL_MINORVERSION% -I.. -I../.. -I../../shared -I%zlib% -I%gmp% -I%zimpldir%\src /LD /MD /Zp8 /Gz /wd4274 -DNDEBUG -D_WINDLL -D_USRDLL -D_CRT_SECURE_NO_DEPRECATE -D_CRT_NONSTDC_NO_DEPRECATE -DWIN32 -DRoleIsExternalLanguageEngine -D"LANGUAGE_ACTIVE=LANGUAGE_ZIMPL" -DZLIB_DLL -D"ZEXPORT=__cdecl" "-Dinline=_inline" "-DSTDC_HEADERS=1" "-DHAVE_STRING_H=1" -DREGEX_MALLOC "-DANSI_PROTOTYPES=1" %src% ../lp_XLI.def %gmp%\release\gmp.lib %zlib%\zdll.lib /Febin\%PLATFORM%\xli_ZIMPL.dll
editbin /LARGEADDRESSAWARE bin\%PLATFORM%\xli_ZIMPL.dll

rem for vs2005 need to embed manifest in dll with manifest tool -  #2 on the next line does this.
rem mt /outputresource:"bin\%PLATFORM%\xli_ZIMPL.dll;#2" /manifest "bin\%PLATFORM%\xli_ZIMPL.dll.manifest"
