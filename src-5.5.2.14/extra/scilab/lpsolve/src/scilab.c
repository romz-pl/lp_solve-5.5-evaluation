...
void Printf(char *format, ...)
{
        ...
        va_start(ap, format);
        l = vsnprintf(buf, sizeof(buf), format, ap) + 1;
        va_end(ap);
        ptr = buf;
        do {
          ptr = strchr(ptr, '\n');
          ...
          if ((ptr > buf) && (ptr[-1] != '\r')) {
            if (l >= sizeof(buf))
              l = sizeof(buf) - 1;
            memmove(ptr + 1, ptr, l - ((int) (ptr - buf)));
            l = l + 1;
            *ptr = '\r';
            ptr++;
          }
...
/* Functions to get len elements from a scilab vector. Matrix
   can be either full or sparse. Elements are stored in indices
   start..start+n-1  Errors out if the scilab vector is not length len */
...
/* Function to get max len elements from a scilab sparse vector. Matrix
   can be either full or sparse. Elements are stored in indices
   start..start+n-1  Errors out if the scilab vector is longer than length len */
