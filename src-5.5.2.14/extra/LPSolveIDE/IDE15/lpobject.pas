...
type
  ...
  TMyBool = (mbFalse, mbTrue, mbAutomatic);
...
const
  ...
  MyBoolMap: array[TMyBool] of integer = (_FALSE, _TRUE, _TRUE + _AUTOMATIC);
  ...
  begin
    case code of
      ...
      _TRUE      : result := mbTrue;
      _TRUE + _AUTOMATIC : result := mbAutomatic;
    else
    ...
    sfMPS:
      begin
        ...
        if NegateObjConstMPS then
          options := options + 32;
        if FFreeMPS then
          result := ChangeLPHandle(read_freeMPS(PChar(filename), options)) else
          result := ChangeLPHandle(read_MPS(PChar(filename), options));
      end;
    sfXLI:
      begin
