...
Private Sub DoReport(ByVal lp As Long, str As String)

    Debug.Print str & " " & Round(lpsolve.time_elapsed(lp), 1) & " sec " & lpsolve.get_working_objective(lp)
...
Private Sub msgfunc(ByVal lp As Long, ByVal userhandle As Long, ByVal message As lpsolve_msgmask)
    ...
    Case MSG_MILPFEASIBLE
        ...
        With lpsolve
            ReDim var(1 To .get_Ncolumns(lp))
            Debug.Print .get_variables(lp, var(1))
        End With
    Case MSG_MILPBETTER
...
Private Sub Test()
    ...
    With lpsolve
        ...
        'These function calls can be used to get the solution in an array
        ...
        .print_str lp, "lp = .read_LP(""lp.lp"", False, ""test"")" & vbLf
